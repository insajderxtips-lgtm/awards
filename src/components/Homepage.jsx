...
import { Shield, Gift, Zap, Trophy, Gamepad2, Star, ArrowRight, LogIn, ChevronDown, Sparkles, Globe } from 'lucide-react';
...
const LanguageSelector = ({ currentLang, setLang }) => {
  ...
  return (
    <div className="relative z-50" ref={dropdownRef}>
      <button onClick={() => setIsOpen(!isOpen)} className="flex items-center gap-2 px-3 py-2 rounded-xl bg-purple-950/40 border border-purple-500/20 text-purple-200 hover:text-white hover:border-fuchsia-500/40 transition-all text-sm font-semibold tracking-wide">
      ...
      {isOpen && (
        <div className="absolute right-0 mt-2 w-40 rounded-xl bg-[#0d041f]/95 border border-fuchsia-500/30 backdrop-blur-xl shadow-[0_10px_30px_rgba(0,0,0,0.5)] overflow-hidden animate-in fade-in slide-in-from-top-2 duration-200">
          <div className="py-1">
            {languages.map((lang) => (
              <button key={lang.code} onClick={() => { setLang(lang.code); setIsOpen(false); }} className={`w-full flex items-center gap-3 px-4 py-2.5 text-sm font-medium transition-colors text-left ${currentLang === lang.code ? 'bg-gradient-to-r from-purple-600/30 to-fuchsia-600/30 text-fuchsia-400 border-l-2 border-fuchsia-500' : 'text-purple-200/70 hover:bg-purple-900/20 hover:text-white'}`}>
                <span>{lang.flag}</span>
                <span>{lang.name}</span>
              </button>
            ))}
...
const Navbar = ({ t, currentLang, setLang }) => {
  ...
  return (
    <nav className={`fixed top-0 left-0 right-0 z-50 transition-all duration-500 ${scrolled ? 'bg-[#090214]/85 backdrop-blur-xl border-b border-fuchsia-500/20 shadow-[0_10px_30px_-10px_rgba(168,85,247,0.1)]' : 'bg-transparent'}`}>
      <div className="max-w-6xl mx-auto px-6 h-20 flex items-center justify-between">
        <div className="flex items-center gap-3 group cursor-pointer" onClick={() => nav('/')}>
        ...
        <div className="flex items-center gap-6 lg:gap-8">
          <div className="hidden md:flex items-center gap-6 lg:gap-8">
            {t.nav.links.map((item) => (
              <button key={item} onClick={() => nav('/login')} className="text-sm font-semibold tracking-wide uppercase text-purple-200/60 hover:text-fuchsia-400 transition-colors duration-300 relative after:absolute after:bottom-[-4px] after:left-0 after:w-0 after:h-[2px] after:bg-fuchsia-500 after:transition-all hover:after:w-full">
                {item}
              </button>
            ))}
          </div>
    ...
    <section className="relative min-h-screen flex items-center justify-center overflow-hidden bg-[#06010f]">
      ...
      <div className="relative z-10 max-w-4xl mx-auto text-center px-6 pt-24">
        ...
        <h1 className="text-4xl md:text-6xl font-black tracking-tight leading-tight mb-6 uppercase">
          <span className="bg-gradient-to-r from-purple-400 via-fuchsia-400 to-violet-500 bg-clip-text text-transparent drop-shadow-[0_2px_10px_rgba(0,0,0,0.5)]">{t.hero.title1}</span><br />
          <span className="text-white relative inline-block drop-shadow-[0_0_30px_rgba(255,255,255,0.1)]">{t.hero.title2}</span>
        </h1>
        <p className="text-purple-200/70 text-lg md:text-xl max-w-2xl mx-auto mb-12 font-medium leading-relaxed">{t.hero.desc}</p>
        <div className="flex flex-col sm:flex-row items-center justify-center gap-5">
          <button onClick={() => nav('/login')} className="w-full sm:w-auto flex items-center justify-center gap-2 px-8 py-4 bg-gradient-to-r from-fuchsia-600 via-purple-600 to-violet-600 hover:from-fuchsia-500 hover:to-violet-500 text-white font-bold tracking-wider uppercase rounded-xl transition-all duration-300 shadow-[0_0_30px_rgba(217,70,239,0.4)] hover:shadow-[0_0_40px_rgba(168,85,247,0.6)] hover:-translate-y-1">
            {t.hero.btnEarn}
            <ArrowRight className="w-5 h-5" />
          </button>
          <button onClick={() => nav('/login')} className="w-full sm:w-auto px-8 py-4 bg-purple-950/20 border border-purple-500/30 hover:border-fuchsia-500/60 text-purple-200 hover:text-white font-bold tracking-wider uppercase rounded-xl transition-all duration-300 backdrop-blur-sm hover:-translate-y-1">{t.hero.btnWorks}</button>
        </div>
        <div className="mt-24 grid grid-cols-2 gap-4 max-w-2xl mx-auto border border-purple-500/10 p-6 rounded-2xl bg-[#0b031a]/45 backdrop-blur-md shadow-[0_20px_50px_rgba(0,0,0,0.4)]">
          <div className="text-center border-r border-purple-500/10"><div className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-purple-400">10K+</div><div className="text-xs font-bold uppercase tracking-widest text-purple-400/60 mt-1">{t.hero.statUsers}</div></div>
          <div className="text-center"><div className="text-3xl md:text-4xl font-black text-transparent bg-clip-text bg-gradient-to-b from-white to-fuchsia-400">150K+</div><div className="text-xs font-bold uppercase tracking-widest text-fuchsia-400/60 mt-1">{t.hero.statClaimed}</div></div>
        </div>
      </div>
    </section>
  );
    ...
    <section className="py-28 px-6 bg-[#090214] relative">
      ...
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tight mb-4">{t.features.heading} <span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-purple-400">LeagueAwards</span></h2>
          <p className="text-purple-300/60 font-medium max-w-xl mx-auto text-lg">{t.features.subheading}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {t.features.items.map((feature, i) => {
            const IconComponent = icons[i] || Star;
            return (
              <button key={feature.title} onClick={() => nav('/login')} className="group p-8 rounded-2xl bg-gradient-to-b from-purple-950/10 to-purple-950/30 border border-purple-900/30 hover:border-fuchsia-500/40 transition-all duration-300 hover:-translate-y-1 hover:shadow-[0_20px_40px_-15px_rgba(168,85,247,0.15)] relative overflow-hidden text-left w-full">
                ...
                <div className="w-12 h-12 rounded-xl bg-gradient-to-br from-purple-950 to-fuchsia-950/60 border border-purple-500/20 group-hover:border-fuchsia-500/50 flex items-center justify-center mb-6 transition-colors shadow-inner"><IconComponent className="w-5 h-5 text-purple-400 group-hover:text-fuchsia-400 group-hover:scale-110 transition-all" /></div>
...
const HowItWorksSection = ({ t }) => {
  ...
  return (
    <section className="py-28 px-6 bg-[#05010d] relative">
      <div className="max-w-6xl mx-auto relative z-10">
        <div className="text-center mb-20">
          <h2 className="text-4xl md:text-5xl font-black text-white uppercase tracking-tight mb-4">{t.howItWorks.heading} <span className="text-transparent bg-clip-text bg-gradient-to-r from-violet-400 to-fuchsia-400">{t.howItWorks.span}</span></h2>
          <p className="text-purple-300/60 font-medium max-w-xl mx-auto text-lg">{t.howItWorks.subheading}</p>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
          {t.howItWorks.steps.map((step, index) => (
            <button key={step.number} onClick={() => nav('/login')} className="relative text-center group w-full">
              <div className="w-16 h-16 rounded-2xl bg-gradient-to-br from-purple-900/20 to-fuchsia-900/20 border border-purple-500/20 group-hover:border-fuchsia-500/50 flex items-center justify-center mx-auto mb-6 transition-all duration-300 shadow-lg group-hover:shadow-[0_0_20px_rgba(217,70,239,0.2)]"><span className="text-xl font-black text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 to-purple-400">{step.number}</span></div>
    ...
    <section className="py-28 px-6 bg-[#090214] relative overflow-hidden">
      ...
      <div className="max-w-4xl mx-auto text-center relative z-10">
        <div className="p-12 md:p-16 rounded-3xl bg-gradient-to-b from-[#120727]/60 to-[#090214]/90 border border-fuchsia-500/20 backdrop-blur-md shadow-[0_30px_70px_rgba(0,0,0,0.5)]">
          <h2 className="text-4xl md:text-6xl font-black text-white uppercase tracking-tighter mb-4">{t.cta.title1} <br className="sm:hidden" /><span className="text-transparent bg-clip-text bg-gradient-to-r from-fuchsia-400 via-purple-400 to-violet-400">{t.cta.title2}</span></h2>
          <p className="text-purple-200/60 text-lg mb-10 max-w-lg mx-auto font-medium">{t.cta.desc}</p>
          <button onClick={() => nav('/login')} className="inline-flex items-center gap-2 px-10 py-4 bg-gradient-to-r from-fuchsia-600 via-purple-600 to-violet-600 hover:from-fuchsia-500 hover:to-violet-500 text-white font-bold tracking-wider uppercase rounded-xl transition-all duration-300 shadow-[0_0_30px_rgba(217,70,239,0.4)] hover:shadow-[0_0_40px_rgba(168,85,247,0.6)] hover:-translate-y-0.5">
            {t.cta.btn}
        ...
        <div className="grid grid-cols-1 md:grid-cols-4 gap-12 mb-12">
          <div className="md:col-span-2">
            <div className="flex items-center gap-3 mb-6">
              <div className="w-8 h-8 rounded-xl bg-gradient-to-br from-fuchsia-500 to-purple-600 flex items-center justify-center shadow-[0_0_15px_rgba(217,70,239,0.3)]"><Shield className="w-4 h-4 text-white" /></div>
              <span className="font-black text-lg uppercase tracking-wider"><span className="text-fuchsia-400">League</span><span className="text-white">Awards</span><span className="text-fuchsia-500 lowercase font-medium">.lol</span></span>
            </div>
            <p className="text-sm text-purple-300/40 max-w-sm leading-relaxed font-medium">{t.footer.affiliation}</p>
          </div>
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-fuchsia-400 mb-4">{t.footer.col1}</h4>
            <ul className="space-y-3">
              {t.nav.links.map((link) => (<li key={link}><button onClick={() => nav('/login')} className="text-sm text-purple-300/60 hover:text-white font-medium transition-colors duration-200">{link}</button></li>))}
            </ul>
          </div>
          <div>
            <h4 className="text-xs font-bold uppercase tracking-widest text-violet-400 mb-4">{t.footer.col2}</h4>
            <ul className="space-y-3">
              {['Contact Us', 'Privacy Policy', 'Terms of Service'].map((link) => (<li key={link}><button onClick={() => nav('/login')} className="text-sm text-purple-300/60 hover:text-white font-medium transition-colors duration-200">{link}</button></li>))}
            </ul>
          </div>
        </div>
        <div className="border-t border-purple-500/5 pt-8 flex flex-col sm:flex-row items-center justify-between gap-4"><p className="text-xs font-medium text-purple-300/30">&copy; {new Date().getFullYear()} LeagueAwards.lol. {t.footer.rights}</p></div>
      </div>
    </footer>
  );
};
